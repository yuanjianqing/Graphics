...
{   
    ...
    p0 = {x - _v[0][0], y - _v[0][1], 0};
    p1 = {x - _v[1][0], y - _v[1][1], 0};
    p2 = {x - _v[2][0], y - _v[2][1], 0};
    float c0, c1, c2;
    c0 = p01.cross(p0)[2];
    c1 = p12.cross(p1)[2];
    c2 = p20.cross(p2)[2];
...
void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer, Primitive type)
{
    ...
    for (auto& i : ind)
    {
        ...
        for (auto & vert : v)
        {
            ...
            vert.z() = vert.z() * f1 + f2;//////////////////////////////////why here do something to z;
        }

        for (int i = 0; i < 3; ++i)
        {
            t.setVertex(i, v[i].head<3>());///////////////////why 3 times
        }
    ...
    for(int i = x_min; i < x_max - 1; i++)
    {
        for(int j = y_min; j < y_max - 1; j++)
        {
            ...
            for(int k = 0; k < 2; k++)
            {
                for(int l = 0; l < 2; l++)
                {
                    if(insideTriangle(i + 0.25 + k * 0.5, j + 0.25 + l * 0.5, t.v))
                    {
                        ...
                        int index = get_index(i * 2 + k, j * 2 + l);
            ...
            if(insNum!= 0)
            {
                ...
                set_pixel(Eigen::Vector3f(i, j, 0), t.getColor() * 0.25 * insNum);
            }
